...
describe("maintenance-cmms-core lifecycle integration", () => {
  const previousStateDir = process.env.GUTU_STATE_DIR;
  let stateDir = "";

  beforeEach(() => {
    stateDir = mkdtempSync(join(tmpdir(), "maintenance-cmms-core-"));
  ...
  it("creates, advances, and reconciles a governed business record", async () => {
    const created = await createPrimaryRecord({
      ...
      actorId: "actor_admin",
      recordId: "maintenance-cmms-core:demo",
      title: "Maintenance & CMMS Core Demo",
      ...
      correlationId: "maintenance-cmms-core:corr",
      processId: "maintenance-lifecycle:demo"
    });
    ...
    const reconciled = await reconcilePrimaryRecord({
      ...
      actorId: "actor_admin",
      recordId: "maintenance-cmms-core:demo",
      exceptionId: "maintenance-cmms-core:exception",
    ...
    const amended = await amendPrimaryRecord({
      ...
      actorId: "actor_admin",
      recordId: "maintenance-cmms-core:demo",
      amendedRecordId: "maintenance-cmms-core:demo:amended",
      expectedRevisionNo: 5,
      title: "Maintenance & CMMS Core Demo Amendment",
      reasonCode: "commercial-correction"
    });
    expect(amended.amendedRecordId).toBe("maintenance-cmms-core:demo:amended");

    const reversed = await reversePrimaryRecord({
      tenantId: "tenant_demo",
      actorId: "actor_admin",
      recordId: "maintenance-cmms-core:demo:amended",
      reversalRecordId: "maintenance-cmms-core:demo:amended:reversal",
      expectedRevisionNo: 1,
      reasonCode: "cancel-amendment"
    });
    expect(reversed.reversalRecordId).toBe("maintenance-cmms-core:demo:amended:reversal");
