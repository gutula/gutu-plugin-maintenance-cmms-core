export type MaintenanceCmmsCoreSqliteOptions = {
  tablePrefix?: string;
};

export function buildMaintenanceCmmsCoreSqliteMigrationSql(options: MaintenanceCmmsCoreSqliteOptions = {}): string[] {
  const tablePrefix = normalizePrefix(options.tablePrefix ?? "maintenance_cmms_core_");
  return [
    ...
    `CREATE UNIQUE INDEX IF NOT EXISTS ${getMaintenanceCmmsCoreSqliteLookupIndexName(tablePrefix)} ON ${tablePrefix}primary_records (tenant_id, title, correlation_id);`,
    `CREATE INDEX IF NOT EXISTS ${getMaintenanceCmmsCoreSqliteStatusIndexName(tablePrefix)} ON ${tablePrefix}exception_records (tenant_id, status, severity);`
  ];
}

export function buildMaintenanceCmmsCoreSqliteRollbackSql(options: MaintenanceCmmsCoreSqliteOptions = {}): string[] {
  const tablePrefix = normalizePrefix(options.tablePrefix ?? "maintenance_cmms_core_");
  return [
...
export function getMaintenanceCmmsCoreSqliteLookupIndexName(tablePrefix = "maintenance_cmms_core_"): string {
  return `${normalizePrefix(tablePrefix)}primary_lookup_idx`;
}

export function getMaintenanceCmmsCoreSqliteStatusIndexName(tablePrefix = "maintenance_cmms_core_"): string {
