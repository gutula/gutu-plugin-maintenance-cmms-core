export type MaintenanceCmmsCoreSqlOptions = {
  schemaName?: string;
  dropSchema?: boolean;
};

export function buildMaintenanceCmmsCoreMigrationSql(options: MaintenanceCmmsCoreSqlOptions = {}): string[] {
  const schemaName = normalizeIdentifier(options.schemaName ?? "maintenance_cmms_core", "schemaName");
  return [
    ...
    `CREATE UNIQUE INDEX IF NOT EXISTS ${getMaintenanceCmmsCoreLookupIndexName()} ON ${schemaName}.primary_records (tenant_id, title, correlation_id);`,
    `CREATE INDEX IF NOT EXISTS ${getMaintenanceCmmsCoreStatusIndexName()} ON ${schemaName}.exception_records (tenant_id, status, severity);`
  ];
}

export function buildMaintenanceCmmsCoreRollbackSql(options: MaintenanceCmmsCoreSqlOptions = {}): string[] {
  const schemaName = normalizeIdentifier(options.schemaName ?? "maintenance_cmms_core", "schemaName");
  const dropSchema = options.dropSchema ?? schemaName !== "maintenance_cmms_core";
  return [
...
export function getMaintenanceCmmsCoreLookupIndexName(): string {
  return "maintenance_cmms_core_primary_lookup_idx";
}

export function getMaintenanceCmmsCoreStatusIndexName(): string {
  return "maintenance_cmms_core_exception_status_idx";
}
